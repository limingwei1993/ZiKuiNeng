...
__interrupt void ScibRxInpur(void)
{
    ...
    for(i=0;i<rev_len;i++)
    {
           ...
           if(revice_len==0 && revice_data==0xeb)
           {
               revice_buff[0]=0xeb;
               revice_len++;
           }
           else if(revice_len==1 && revice_data==0x90)
           {
               revice_buff[1]=0x90;
               revice_len++;
           }
           else if(revice_len>1)
           {
               ...
               if(revice_len>=10)
               {
                   if(CRC_Get_Cumulative_Sum(revice_buff, 9)== revice_buff[9])
                   {
                       /*message process*/
...
uint8_t GetFrameFromQueue(SCI_RecvQueue *q)
{
  ...
  while (!SCI_RecvQueueEmpty(q) && !res)
  {
    ...
    else if (q->frame_len > 1)
    {
      ...
      if (q->frame_len >= 10)
      {
        if (q->frame_temp[9] == CRC_Get_Cumulative_Sum(q->frame_temp, 9)) /*crc校验通过*/
        {
          res = q->frame_len;
        }
        q->frame_len = 0;
